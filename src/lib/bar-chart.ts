import type { ColourEntry, BlacklistEntry } from './aggregation-engine.ts'
...
export class BarChart {
    ...
    render(included: ColourEntry[], excluded: BlacklistEntry[], bucketSize: number): void {
    ...
    private buildExcludedRow(entry: BlacklistEntry): HTMLElement {
        const row = document.createElement('div')
        ...
        const keys = this.engine.rawKeysForEntry(entry)
        ...
        const percentage = this.imageProcessor.totalPixels > 0
            ...
            : 0
        row.appendChild(this.buildBar(entry.hex, entry.isTransparent, percentage))
