...
describe('ImageProcessor.loadFile validation', () => {
  it('rejects GIF MIME type', async () => {
    const processor = new ImageProcessor();
    const file = new File([], 'anim.gif', { type: 'image/gif' });
    await expect(processor.loadFile(file)).rejects.toThrow(ValidationError);
    await expect(processor.loadFile(file)).rejects.toThrow(
  ...
  it('rejects oversized file', async () => {
    ...
    await expect(processor.loadFile(file)).rejects.toThrow(ValidationError);
    await expect(processor.loadFile(file)).rejects.toThrow('File is too large. Maximum size is 16 MB.');
  });
  ...
  it('does not reject file at exactly 16 MB for size', async () => {
    ...
    await expect(processor.loadFile(file)).rejects.not.toBeInstanceOf(ValidationError);
  });
});
