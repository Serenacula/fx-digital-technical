...
- `reaggregate(map: RawMap, bucketSize: number): AggregatedMap` — takes raw `Record<"r,g,b", count>` map and produces a new map with keys rounded to `bucketSize`
- `toHex(r: number, g: number, b: number): string` — formats `#RRGGBB`
- `sortedColours(map: AggregatedMap, totalPixels: number): ColourEntry[]` — returns array of `{ hex, count, percentage }` sorted descending by count
- Type definitions for `RawMap`, `AggregatedMap`, `ColourEntry`
